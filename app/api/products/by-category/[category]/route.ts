...
import { getProductsByCategory, getCategoryBySlug, validatePaginationParams } from '@/lib/product-service';
import type { ApiResponse, PaginatedResponse, ProductWithCategory, ProductCategory } from '@/types';
...
export async function GET(
  ...
  try {
    ...
    const rawLimit = limitParam ? parseInt(limitParam, 10) : SITE_CONFIG.pagination.defaultLimit;
    ...
    if (isNaN(rawLimit) || rawLimit < SITE_CONFIG.pagination.minLimit || rawLimit > SITE_CONFIG.pagination.maxLimit) {
      return Response.json(
        { 
          success: false,
    ...
    const apiResponse: ApiResponse<PaginatedResponse<ProductWithCategory>> = {
      success: true,
      data: {
