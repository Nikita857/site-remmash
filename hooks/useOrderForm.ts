...
const orderFormSchema = z.object({
  ...
  phone: z.string()
    ...
    .refine(phone => {
      const cleanPhone = phone.replace(VALIDATION_CONFIG.PHONE.CLEAN_PATTERN, '');
    ...
    .refine(phone => {
      ...
      message: 'Номер телефона может содержать только цифры, пробелы, +, -, (, )'
    }),
  ...
  comment: z.string()
    ...
    .refine(comment => !comment || comment.length <= VALIDATION_CONFIG.COMMENT.MAX, {
...
const orderFormWithConsentSchema = orderFormSchema.extend({
  consent: z.boolean().refine(val => val === true, {
    message: 'Необходимо подтвердить согласие с политикой обработки персональных данных'
  })
});
...
interface UseOrderFormReturn {
  ...
  submitOrder: (additionalData?: Partial<OrderFormData>, withConsent?: boolean) => Promise<ServiceResult<unknown>>;
  resetForm: () => void;
  clearError: () => void;
}
...
export function useOrderForm({
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof OrderFormData, string>> & { consent?: string }>({});
  const [touched, setTouched] = useState<Partial<Record<keyof OrderFormData, boolean>>>({});
  const [dirty, setDirty] = useState<Partial<Record<keyof OrderFormData, boolean>>>({});
  ...
  const recordSubmission = useCallback(() => {
    ...
    try {
      ...
      localStorage.setItem(SITE_CONFIG.rateLimit.STORAGE_KEY, JSON.stringify(record));
  ...
  const validateField = useCallback((field: keyof OrderFormData): boolean => {
    try {
      orderFormSchema.pick({ [field]: true }).parse({ [field]: formData[field] });
  ...
  const handleChange = useCallback((name: keyof OrderFormData, value: string) => {
    setFormData(prev => {
  ...
  const handleBlur = useCallback((name: keyof OrderFormData) => {
  ...
  const validateForm = useCallback((withConsent: boolean = false): boolean => {
    try {
      ...
      if (err instanceof z.ZodError) {
        const newErrors: Partial<Record<keyof OrderFormData, string>> & { consent?: string } = {};
  ...
  const submitOrder = useCallback(async (
    ...
    if (withConsent && !consent) {
      const errorMsg = 'Необходимо подтвердить согласие с политикой обработки персональных данных';
      setError(errorMsg);
      onError?.(errorMsg);
      return {
        success: false,
  ...
  }, [formData, consent, validateForm, onSuccess, onError, isRateLimited, recordSubmission, checkRateLimit]);
